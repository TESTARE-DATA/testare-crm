    ...
    <div className="mx-auto max-w-[1080px] fade-up">
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[240px_1fr]">
        ...
        <div className="flex flex-col items-center gap-4">
          <Phone st={st} screen={screen} setScreen={setScreen} clientName={clientName} clientLogo={clientLogo} />
          <p className="max-w-md text-center text-[12px] text-muted-2">Questo è ciò che vede l&apos;atleta dalla sua app: compila il check-in al risveglio, riceve un feedback semplice sulla prontezza e può consultare il suo carico. Gli alert e i dati completi restano allo staff.</p>
        </div>
      </div>
    </div>
  );
...
function Phone({ st, screen, setScreen, clientName, clientLogo }: { st: ReadinessState; screen: Screen; setScreen: (s: Screen) => void; clientName: string; clientLogo: string }) {
  return (
    <div className="relative w-[380px] max-w-full shrink-0 rounded-[2.4rem] border border-border bg-[#0b1220] p-2.5 shadow-xl">
      <div className="relative flex h-[720px] flex-col overflow-hidden rounded-[2rem] bg-background">
        ...
        <div className="flex-1 overflow-y-auto pt-8">
          {screen === "oggi" && <ScreenOggi st={st} clientName={clientName} clientLogo={clientLogo} onCheckin={() => setScreen("checkin")} onLoad={() => setScreen("carico")} />}
          {screen === "checkin" && <ScreenCheckin key={st.athlete.id} st={st} />}
          {screen === "carico" && <ScreenCarico st={st} />}
        </div>
        {/* Bottom nav */}
...
function ScreenOggi({ st, clientName, clientLogo, onCheckin, onLoad }: { st: ReadinessState; clientName: string; clientLogo: string; onCheckin: () => void; onLoad: () => void }) {
  ...
  return (
    <div className="space-y-4 px-4 pb-4">
      ...
      {/* Red flag localizzato (visibile all'atleta in forma soft) */}
      {st.clinicalFlag && (
      ...
      {/* Check-in di oggi */}
      <button onClick={onCheckin} className="flex w-full items-center gap-3 rounded-xl border border-border bg-surface px-3.5 py-3 text-left transition-colors hover:bg-background">
        <span className="brand-soft-bg brand-text flex h-9 w-9 shrink-0 items-center justify-center rounded-lg"><Icon name="clipboard" size={17} /></span>
        <span className="min-w-0 flex-1">
          <span className="block text-[13px] font-semibold">Check-in di oggi</span>
          <span className="block text-[11px] text-muted-2">{st.entry ? "Completato — tocca per rivederlo" : "Da compilare al risveglio"}</span>
        </span>
        {st.entry ? <span className="text-[var(--good)]"><Icon name="link" size={16} /></span> : <Icon name="chevron" size={16} className="text-muted-2" />}
      </button>

      {/* Carico settimana */}
      <button onClick={onLoad} className="flex w-full items-center gap-3 rounded-xl border border-border bg-surface px-3.5 py-3 text-left transition-colors hover:bg-background">
        <span className="brand-soft-bg brand-text flex h-9 w-9 shrink-0 items-center justify-center rounded-lg"><Icon name="load" size={17} /></span>
        <span className="min-w-0 flex-1">
          <span className="block text-[13px] font-semibold">Carico della settimana</span>
          <span className="block text-[11px] text-muted-2">{st.load.weekly.toLocaleString("it-IT")} A.U. · sedute registrate</span>
        </span>
        <span className="text-[12px] font-bold" style={{ color: wow > 15 ? "var(--warn)" : wow < -15 ? "var(--elite)" : "var(--muted)" }}>{wow > 0 ? "+" : ""}{wow}%</span>
      </button>
...
function ScreenCheckin({ st }: { st: ReadinessState }) {
  const init = useMemo(() => {
  ...
  return (
    <div className="space-y-4 px-4 pb-6">
        ...
        <div key={q.key} className="rounded-xl border border-border p-3">
          ...
          {q.kind === "hours" ? (
            <div className="flex items-center gap-3">
              <input type="range" min={3.5} max={10} step={0.5} value={vals[q.key]} onChange={(e) => set(q.key, Number(e.target.value))} className="flex-1 accent-[var(--brand-primary)]" style={{ accentColor: q.color }} />
              <span className="w-14 text-right font-mono text-[15px] font-bold" style={{ color: q.color }}>{vals[q.key].toFixed(1)}h</span>
            </div>
          ) : (
            <>
      ...
      <button onClick={() => setSent(true)} className="brand-bg brand-on w-full rounded-xl py-3 text-[14px] font-bold shadow-sm transition-transform hover:scale-[1.01]">
        {sent ? "✓ Inviato" : "Invia il check-in"}
      </button>
      {sent && <p className="text-center text-[11.5px] text-[var(--good)]">Grazie! Il tuo check-in è stato registrato (anteprima).</p>}
    </div>
  );
}

function ScreenCarico({ st }: { st: ReadinessState }) {
  ...
  return (
    <div className="space-y-4 px-4 pb-6">
        ...
        <div className="flex items-end justify-between gap-1.5" style={{ height: 90 }}>
          {last7.map((d) => (
            <div key={d.date} className="flex flex-1 flex-col items-center gap-1">
              <div className="w-full rounded-t bg-[var(--brand-primary)]" style={{ height: `${(d.dailyLoad / maxLoad) * 70}px`, minHeight: 2, opacity: d.dailyLoad ? 1 : 0.25 }} />
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-xl border border-border p-3">
          <div className="text-[10.5px] font-semibold uppercase tracking-wide text-muted-2">Carico acuto</div>
          <div className="mt-1 text-xl font-extrabold">{st.load.ewmaAcute.toLocaleString("it-IT")}</div>
          <div className="text-[10px] text-muted-2">EWMA 7 gg</div>
        </div>
        <div className="rounded-xl border border-border p-3">
          <div className="text-[10.5px] font-semibold uppercase tracking-wide text-muted-2">Carico cronico</div>
          <div className="mt-1 text-xl font-extrabold">{st.load.ewmaChronic.toLocaleString("it-IT")}</div>
          <div className="text-[10px] text-muted-2">EWMA 28 gg</div>
        </div>
      </div>
